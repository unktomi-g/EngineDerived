...
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnJoystick, float, float);
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnTouchEvent, FGeometry, const FPointerEvent&);

/**
 * A virtual joystsick
 */

class JOYSTICKWIDGETPLUGIN_API SVirtualJoystickWidget : public SLeafWidget
{
...
public:
	...
	SLATE_BEGIN_ARGS( SVirtualJoystickWidget )
		{}
	SLATE_END_ARGS()
	 ...
	 * Note that this function is also used internally, so even if this returns false but an SVirtualJoystickWidget
	...
	FOnTouchEvent OnTouchStartedEvent;
	FOnTouchEvent OnTouchMovedEvent;
	FOnTouchEvent OnTouchEndedEvent;
	FOnTouchEvent OnTouchGestureEvent;
