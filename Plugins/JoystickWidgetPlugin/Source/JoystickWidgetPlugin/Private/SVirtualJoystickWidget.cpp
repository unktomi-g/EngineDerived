...
void SVirtualJoystickWidget::Construct( const FArguments& InArgs )
{
	...
	FSlateApplication::Get().GetPlatformApplication()->OnDisplayMetricsChanged().AddSP(this, &SVirtualJoystickWidget::HandleDisplayMetricsChanged);
}

void SVirtualJoystickWidget::HandleDisplayMetricsChanged(const FDisplayMetrics& NewDisplayMetric)
{
...
void SVirtualJoystickWidget::SetGlobalParameters(float InActiveOpacity, float InInactiveOpacity, float InTimeUntilDeactive, float InTimeUntilReset, float InActivationDelay, bool InbPreventReCenter, float InStartupDelay)
{
...
bool SVirtualJoystickWidget::ShouldDisplayTouchInterface()
{
	return true;
}
...
int32 SVirtualJoystickWidget::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
...
FVector2D SVirtualJoystickWidget::ComputeDesiredSize( float ) const
{
	return FVector2D(100, 100);
}

bool SVirtualJoystickWidget::SupportsKeyboardFocus() const
{
	return false;
}

FReply SVirtualJoystickWidget::OnTouchStarted(const FGeometry& MyGeometry, const FPointerEvent& Event)
{
		...
		if (Control.CapturedPointerIndex == -1)
		{
			if (PositionIsInside(Control.CorrectedCenter, LocalCoord, Control.CorrectedInteractionSize))
			{
				...
				if (ActivationDelay == 0.f)
				{
					...
					{
						OnTouchStartedEvent.Broadcast(MyGeometry, Event);
...
FReply SVirtualJoystickWidget::OnTouchMoved(const FGeometry& MyGeometry, const FPointerEvent& Event)
{
	FVector2D LocalCoord = MyGeometry.AbsoluteToLocal( Event.GetScreenSpacePosition() );
	for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
	{
		...
		if (Control.CapturedPointerIndex == Event.GetPointerIndex())
		{
			...
			{
				OnTouchMovedEvent.Broadcast(MyGeometry, Event);
				return FReply::Unhandled();
			}
...
FReply SVirtualJoystickWidget::OnTouchEnded(const FGeometry& MyGeometry, const FPointerEvent& Event)
{
	for ( int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++ )
	{
		FControlInfo& Control = Controls[ControlIndex];
		...
		if ( Control.CapturedPointerIndex == Event.GetPointerIndex() )
		{
			...
			if ( Control.bNeedUpdatedCenter )
			{
				...
				OnTouchEndedEvent.Broadcast(MyGeometry, Event);
				return FReply::Unhandled();
			}
			OnTouchEndedEvent.Broadcast(MyGeometry, Event);
...
bool SVirtualJoystickWidget::HandleTouch(int32 ControlIndex, const FVector2D& LocalCoord, const FVector2D& ScreenSize)
{
	FControlInfo& Control = Controls[ControlIndex];
...
void SVirtualJoystickWidget::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	for (int32 ControlIndex = 0; ControlIndex < Controls.Num(); ControlIndex++)
	{
		...
		if (Control.CapturedPointerIndex >= 0 || Control.bSendOneMoreEvent)
		{
			...
			OnJoystick.Broadcast(NormalizedOffset.X, -NormalizedOffset.Y);
		}
...
void SVirtualJoystickWidget::SetJoystickVisibility(const bool bInVisible, const bool bInFade)
{
...
void SVirtualJoystickWidget::AlignBoxIntoScreen(FVector2D& Position, const FVector2D& Size, const FVector2D& ScreenSize)
{
